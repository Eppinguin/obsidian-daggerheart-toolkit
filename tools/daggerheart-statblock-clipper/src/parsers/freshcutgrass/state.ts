/** FreshCutGrass MAIN-world state collector.
 *
 * Ported verbatim from the former `freshcutgrass-state.js`.
 *
 * IMPORTANT: this function is serialized into the page's MAIN world via
 * `chrome.scripting.executeScript({ func })`, so it must stay entirely
 * self-contained — no imports, no references to module scope. Every helper it
 * needs is declared inside it.
 */
 ...
 * returned.
 */
export function collectFreshCutGrassState(targetId = '') {
    ...
    function objectScore(value: any) {
        ...
        for (const key of keys.slice(0, 100)) {
            ...
            if (normalized === 'name' || normalized === 'title')
                score += typeof entry === 'string' && entry.trim() ? 5 : 0;
            else if (normalized === 'features' || normalized === 'feats' || normalized === 'abilities')
                score += Array.isArray(entry) || (entry && typeof entry === 'object') ? 6 : 0;
                ...
                score += entry != null ? 2 : 0;
            else score += entry != null ? 1 : 0;
        }
        return score + (hasTarget ? 50 : 0);
    }

    function safeCopy(value: any, depth = 0, copied = new WeakSet()): any {
        ...
        if (Array.isArray(value)) {
            return value
                .slice(0, 100)
                .map((entry) => safeCopy(entry, depth + 1, copied))
        ...
        for (const key of keys.slice(0, 120)) {
            if (/^(ownerDocument|parentNode|parentElement|stateNode|return|child|sibling|alternate)$/i.test(key))
                continue;
            let entry;
            try {
                entry = value[key];
            ...
            if (copiedEntry !== undefined) (output as any)[key] = copiedEntry;
        }
        return output;
    }

    function addCandidate(value: any, path: any, score: any) {
    ...
    function walk(value: any, path = 'root', depth = 0, targetContext = false) {
        ...
        if (targetId) {
            localTarget =
                ...
                keys.some((key) => {
                    try {
                        return scalar(value[key]) && String(value[key]) === targetId;
                    } catch (_error) {
                        return false;
                    }
                });
    ...
    for (const node of nodes) {
        ...
        for (const prop of props) {
            if (/^__(reactProps|reactFiber|vueParentComponent|vue__|svelte)/i.test(prop)) {
                try {
                    walk((node as any)[prop], `dom.${prop}`);
                } catch (_error) {
    ...
    for (const key of globals) {
        try {
            if ((window as any)[key]) walk((window as any)[key], `window.${key}`);
        } catch (_error) {
            /* ignore */
        }
    }
    ...
    for (const storage of [window.localStorage, window.sessionStorage]) {
        try {
            for (let index = 0; index < Math.min(storage.length, 200); index += 1) {
                ...
                try {
                    walk(JSON.parse(raw), `storage.${key}`);
                } catch (_error) {
                    /* not JSON */
                }
            }
    ...
    for (const candidate of candidates) {
        let signature;
        try {
            signature = JSON.stringify(candidate.value).slice(0, 5000);
        } catch (_error) {
            continue;
        }
    ...
    return {
        ...
        resourceUrls: performance
            ...
            .filter((url) => /firebase|firestore|homebrew|adversar/i.test(url))
            .slice(0, 50),
    };
}
