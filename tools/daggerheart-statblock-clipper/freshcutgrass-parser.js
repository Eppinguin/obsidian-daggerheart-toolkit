(() => {
  ...
  if (!base || base.__freshCutGrassStatePatch) return;

  const clean = (value) => String(value ?? '').replace(/\u00a0/g, ' ').replace(/[ \t]+/g, ' ').replace(/\s*\n\s*/g, ' ').trim();
  function quality(item) {
    return (item?.name && item.name !== 'Untitled Statblock' ? 5 : 0) + (item?.tier != null ? 2 : 0) +
      (item?.difficulty != null ? 2 : 0) + (item?.hp != null || item?.impulses || item?.adversaries ? 2 : 0) +
      (item?.features?.length ? 2 : 0) + (item?.weapon || item?.type ? 1 : 0);
  }
  ...
  function textFrom(value) {
    ...
    if (typeof value === 'object') {
      ...
      if (name != null && modifier != null && String(name) !== String(modifier)) {
        ...
        return clean(`${textFrom(name)}${parsed != null ? ` ${parsed >= 0 ? '+' : ''}${parsed}` : ` ${textFrom(modifier)}`}`);
      }
      const known = directValue(value, ['text', 'description', 'desc', 'value', 'name', 'title']);
      if (known != null) return textFrom(known);
      const entries = Object.entries(value).filter(([, entry]) => entry != null && (typeof entry === 'string' || typeof entry === 'number'));
  ...
  function objectStatScore(object, targetId = '', targetContext = false) {
    ...
    if (directValue(object, ['thresholds', 'damageThresholds', 'majorThreshold', 'severeThreshold']) != null) score += 2;
    return score;
  }

  function enumerateStateObjects(input, targetId = '') {
    const output = [];
    const seen = new WeakSet();
    const roots = Array.isArray(input?.candidates) ? input.candidates.map((entry) => entry?.value ?? entry) : (Array.isArray(input) ? input : [input]);

    function containsTargetShallow(object) {
      if (!targetId || !object || typeof object !== 'object') return false;
      return Object.values(object).some((value) => (typeof value === 'string' || typeof value === 'number') && String(value) === targetId);
    }

    function visit(value, depth = 0, targetContext = false, path = 'root') {
      if (!value || typeof value !== 'object' || seen.has(value) || depth > 12) return;
      seen.add(value);
      const localTarget = targetContext || containsTargetShallow(value);
      if (!Array.isArray(value)) output.push({ object: value, score: objectStatScore(value, targetId, localTarget), targetContext: localTarget, path });
    ...
    roots.forEach((root, index) => visit(root, 0, false, `candidate[${index}]`));
    return output.sort((a, b) => b.score - a.score);
  }

  function normalizeStateCost(feature) {
  ...
  function normalizeStateFeature(feature, forcedType = '') {
    ...
    let type = textFrom(directValue(feature, ['type', 'featureType', 'actionType', 'category'])) || forcedType || 'Feature';
    ...
    if (inline) { name = clean(inline[1]); type = inline[2]; }
    if (!name && desc) {
      const split = desc.match(/^([^:]{2,100})\s*:\s*(.+)$/);
      if (split) { name = clean(split[1]); desc = clean(split[2]); }
    }
    if (!name || !desc) return null;
    const parsedCost = normalizeStateCost(feature);
    return { name, type: /passive/i.test(type) ? 'Passive' : /reaction/i.test(type) ? 'Reaction' : /action/i.test(type) ? 'Action' : clean(type), ...(parsedCost ? { parsedCost } : {}), desc };
  }

  function normalizeStateFeatures(value) {
    const output = [];
    if (Array.isArray(value)) {
      ...
      for (const [key, entries] of Object.entries(value)) {
        const forcedType = /passive/i.test(key) ? 'Passive' : /reaction/i.test(key) ? 'Reaction' : /action/i.test(key) ? 'Action' : '';
  ...
  function stateThresholds(object) {
    ...
    const severe = numberFrom(directValue(thresholdObject, ['severe', 'severeHp', 'severeThreshold', 'severe_hp', 'second']));
    return major != null || severe != null ? `${major ?? '—'}/${severe ?? '—'}` : '';
  }

  function stateAttack(object) {
    const nested = directValue(object, ['standardAttack', 'attackData', 'primaryAttack']);
    const attackObject = nested && typeof nested === 'object' ? nested : (directValue(object, ['attack']) && typeof directValue(object, ['attack']) === 'object' ? directValue(object, ['attack']) : {});
    const rootAttack = directValue(object, ['attack']);
    const weapon = textFrom(directValue(attackObject, ['name', 'title', 'weapon', 'attackName'])) ||
      textFrom(directValue(object, ['attackName', 'weapon', 'standardAttackName'])) || (typeof rootAttack === 'string' && !/^[+−-]?\d+$/.test(rootAttack) ? clean(rootAttack) : '');
    const range = textFrom(directValue(attackObject, ['range', 'attackRange'])) || textFrom(directValue(object, ['range', 'attackRange']));
    let damage = textFrom(directValue(attackObject, ['damage', 'damageDice', 'damageRoll'])) || textFrom(directValue(object, ['damage', 'damageDice', 'damageRoll']));
    const damageType = textFrom(directValue(attackObject, ['damageType', 'type'])) || textFrom(directValue(object, ['damageType', 'physicalOrMagical']));
    if (damageType && damage && !new RegExp(`\\b${damageType}\\b`, 'i').test(damage)) damage = clean(`${damage} ${damageType}`);
    const modifierValue = directValue(attackObject, ['modifier', 'attackModifier', 'attackMod', 'atk']) ?? directValue(object, ['attackModifier', 'attackMod', 'atk']);
    const modifierNumber = numberFrom(modifierValue ?? (typeof rootAttack !== 'object' ? rootAttack : undefined));
    const modifier = modifierNumber != null ? `${modifierNumber >= 0 ? '+' : ''}${modifierNumber}` : textFrom(modifierValue);
    return { weapon, range, damage, attack: modifier };
  }

  function normalizeStateObject(object, sourceUrl, targetId = '', targetContext = false) {
    ...
    const features = normalizeStateFeatures(directValue(object, ['features', 'feats', 'abilities', 'actions', 'featureGroups']));
    if (!name || (tier == null && difficulty == null && !features.length)) return null;

    const location = (() => { try { return new URL(sourceUrl); } catch (_error) { return null; } })();
    const result = {
      name,
      tier,
      type,
      ...
      author: textFrom(directValue(object, ['author', 'authorName', 'creator', 'creatorName', 'username', 'ownerName'])),
      extractedAt: new Date().toISOString(),
      extractionMethod: 'freshcutgrass-app-state'
    };

    const isEnvironment = /environment/i.test(textFrom(directValue(object, ['category', 'kind', '_type']))) ||
      /^(Traversal|Event|Exploration)$/i.test(type) || directValue(object, ['impulses', 'potentialAdversaries', 'toneAndFeel']) != null;
    if (isEnvironment) {
      result.tone = textFrom(directValue(object, ['toneAndFeel', 'tone_and_feel', 'tone', 'feel']));
      result.adversaries = textFrom(directValue(object, ['potentialAdversaries', 'potential_adversaries', 'adversaries']));
      ...
      result.motives = textFrom(directValue(object, ['motivesAndTactics', 'motives_and_tactics', 'motivesTactics', 'motives']));
    }
  ...
  function mergeStatblocks(primary, fallback) {
    if (!fallback) return primary;
    const result = { ...fallback, ...primary };
    for (const key of ['desc', 'type', 'weapon', 'range', 'damage', 'attack', 'thresholds', 'xp', 'motives', 'tone', 'adversaries', 'impulses', 'author']) {
    ...
    if ((!primary.features || primary.features.length < (fallback.features?.length || 0)) && fallback.features?.length) result.features = fallback.features;
  ...
  function parseFreshCutGrassState(input, sourceUrl = '', domItems = []) {
    const targetId = (() => { try { return new URL(sourceUrl).searchParams.get('id') || input?.targetId || ''; } catch (_error) { return input?.targetId || ''; } })();
    ...
    for (const entry of objects) {
      ...
      seen.add(key);
      const fallback = domItems.find((candidate) => clean(candidate.name).toLowerCase() === clean(item.name).toLowerCase()) || (domItems.length === 1 ? domItems[0] : null);
      parsed.push({ item, fallback, score: item.__stateScore || entry.score, target: Boolean(item.__targetContext) });
    }
    const selected = targetId && parsed.some((entry) => entry.target)
      ? parsed.filter((entry) => entry.target)
      : parsed;
  ...
  globalThis.DHStatblockParser = {
    ...base,
    __freshCutGrassStatePatch: true,
