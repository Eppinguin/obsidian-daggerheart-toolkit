...
const adapter = fs.readFileSync(path.join(root, 'statblock-format-adapter.js'), 'utf8');
const background = fs.readFileSync(path.join(root, 'background.js'), 'utf8');
const baseManifest = JSON.parse(fs.readFileSync(path.join(root, 'manifests/base.json'), 'utf8'));
const chromiumManifest = JSON.parse(fs.readFileSync(path.join(root, 'manifests/chromium.json'), 'utf8'));
...
assert.match(adapter, /DH_OPEN_EXTERNAL_URI/);
assert.match(adapter, /active:\s*true/);
assert.match(background, /windows\.onFocusChanged/);
assert.match(background, /restoreSourceTab/);
