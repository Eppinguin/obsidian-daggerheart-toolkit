...
assert.deepEqual(
  { weapon: renderedShadowHag.weapon, range: renderedShadowHag.range, damage: renderedShadowHag.damage, attack: renderedShadowHag.attack },
  { weapon: 'Moon Staff', range: 'Far', damage: '2d10+3 Magical', attack: '+2' }
);
...
assert.equal(renderedShadowHag.features[1].desc, 'The Hag can be spotlighted up to three times per GM turn. Spend Fear as usual to spotlight them.');
assert.equal(renderedShadowHag.features.find((feature) => feature.name === 'Raking Claws').parsedCost, 'Stress 1');
assert.equal(renderedShadowHag.features.find((feature) => feature.name === 'Waking Nightmare').parsedCost, 'Fear 1');
assert.equal(renderedShadowHag.features.find((feature) => feature.name === 'Prey on Terror').parsedCost, 'Stress 1');
assert.ok(!renderedShadowHag.features.some((feature) => /MINOR|MAJOR|SEVERE|LIKED|COMMENTS|refleximage/i.test(feature.name)));
...
console.log('FreshCutGrass rendered Shadow Hag repair regression passed');
...
console.log('FreshCutGrass attack-heading and comment-description regressions passed');
