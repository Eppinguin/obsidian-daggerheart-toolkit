...
(function installDaggerheartStatblockFormat(root) {
  ...
  const ENVIRONMENT_ROLES = new Set(['traversal', 'event', 'exploration', 'environmenttraversal', 'environmentevent', 'environmentexploration', 'environmentsocial']);
  ...
  function normalizeCost(value) {
    ...
    const match = text.match(/^(fear|stress|hope)\s*:?\s*(\d+)$/i);
    return match ? `${match[1][0].toUpperCase()}${match[1].slice(1).toLowerCase()} ${match[2]}` : text;
  }
  ...
  function normalizeFeature(feature) {
    ...
    const parsedCost = normalizeCost(feature.parsedCost || feature.cost || (costMatch ? `${costMatch[1]} ${costMatch[2]}` : ''));
    ...
    return {
      ...
      ...(feature.countdown !== undefined && feature.countdown !== null && feature.countdown !== '' ? { countdown: feature.countdown } : {}),
      description
    };
  }
  ...
  function normalizeStatblock(input, declaredType) {
    ...
    Object.keys(source).forEach((key) => { if (!source[key]) delete source[key]; });

    const features = (Array.isArray(input.features) ? input.features : Array.isArray(input.feats) ? input.feats : [])
    ...
    const result = {
      ...
      ...(input.tier !== undefined && input.tier !== '' ? { tier: numberOrNull(input.tier) ?? clean(input.tier) } : {}),
      ...(clean(input.type) ? { type: clean(input.type) } : {}),
      ...(clean(input.description || input.desc) ? { description: clean(input.description || input.desc) } : {}),
      ...(input.difficulty !== undefined && input.difficulty !== '' ? { difficulty: numberOrNull(input.difficulty) ?? clean(input.difficulty) } : {}),
      hp_stress: {
    ...
    if (category === 'adversary') {
      ...
      const attack = {
        name: clean(attackObject?.name || input.weapon || (!attackObject && typeof input.attack === 'string' && !/^[+−-]?\d+$/.test(clean(input.attack)) ? input.attack : '') || 'Attack'),
      ...
      const experience = input.experience ?? input.xp;
      if (experience !== undefined && experience !== null && clean(typeof experience === 'string' ? experience : JSON.stringify(experience))) result.experience = experience;
      const motives = input.motives_tactics ?? input.motives_and_tactics ?? input.motives;
      if (clean(Array.isArray(motives) ? motives.join(', ') : motives)) result.motives_tactics = Array.isArray(motives) ? motives : clean(motives);
    } else {
  ...
  function normalizePayload(payload) {
    ...
    if (parsed?.type && parsed?.data !== undefined) {
      ...
      for (const item of items) {
        ...
        if (type === 'adversary' || type === 'environment') {
          ...
          if (validation.data) entries.push({ type, version: parsed.version || FORMAT_VERSION, exportDate: parsed.exportDate || new Date().toISOString(), data: validation.data, validation });
        } else {
          entries.push({ type, version: parsed.version || FORMAT_VERSION, exportDate: parsed.exportDate || new Date().toISOString(), data: item, validation: { valid: true, errors: [], warnings: [] } });
        }
      }
      return entries;
    }
    ...
    for (const item of items) {
      ...
      if (type === 'adversary' || type === 'environment') {
        const validation = validateStatblock({ ...item, category: type });
        if (validation.data) entries.push({ type, version: FORMAT_VERSION, exportDate: new Date().toISOString(), data: validation.data, validation });
      } else if (type !== 'unknown') {
        entries.push({ type, version: FORMAT_VERSION, exportDate: new Date().toISOString(), data: item, validation: { valid: true, errors: [], warnings: [] } });
      }
    }
    return entries;
  }
  ...
  const api = { FORMAT_VERSION, clean, normalizeFeature, normalizeStatblock, validateStatblock, detectContentType, createEnvelope, normalizePayload, toYaml, toMarkdown, toJson };
