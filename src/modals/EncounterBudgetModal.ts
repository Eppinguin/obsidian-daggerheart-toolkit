...
export class EncounterBudgetModal extends Modal {
    ...
    onOpen() {
            ...
            .addText((text) =>
                text
                    ...
                    .onChange((value) => {
                        ...
                        if (!isNaN(count) && count >= 0) this.config.playerCount = count;
                    }),
            );
        ...
        new Setting(contentEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(this.config.isEasier).onChange((value) => {
                    this.config.isEasier = value;
                    if (value) this.config.isHarder = false;
                    this.onOpen();
                }),
            );
        new Setting(contentEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(this.config.isHarder).onChange((value) => {
                    this.config.isHarder = value;
                    if (value) this.config.isEasier = false;
                    this.onOpen();
                }),
            );
        new Setting(contentEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(this.config.isDamageBoosted).onChange((value) => (this.config.isDamageBoosted = value)),
            );
        new Setting(contentEl)
            .setName('Lower Tier Adversary (+1)')
            .setDesc('Applies if you choose an adversary from a lower tier than the party.')
            .addToggle((toggle) =>
                toggle.setValue(this.config.useLowerTier).onChange((value) => (this.config.useLowerTier = value)),
            );
